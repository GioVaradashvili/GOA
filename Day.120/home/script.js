class OrderManager {
  ...
  processOrder(orderId, customerName, callback) {
    // დავამატოთ შეკვეთა activeOrders-ში
    this.activeOrders.set(orderId, customerName);
    console.log(`➡️ მიღებულია ახალი შეკვეთა #${orderId}, მომხმარებელი: ${customerName}`);

    // 3 წამში შესრულდეს შეკვეთა
    setTimeout(() => {
      // ამოშლა activeOrders-დან
      this.activeOrders.delete(orderId);

      // completedOrders-ში დამატება
